...
const createTutorProfile = async (userId, payload) => {
    ...
    try {
        ...
        const tutorProfile = await prisma.$transaction(async (tx) => {
            ...
            if (existingProfile)
                throw new Error("User already has tutor profile");
            ...
            if (!profile)
                throw new Error("Tutor profile not created");
            //update user role
...
const getAllTutors = async (options = {}) => {
...
const getTutorOwnProfile = async (userId) => {
    console.log(userId);
    return await prisma.tutorProfile.findUnique({
        where: { userId },
        include: tutorIncludeConfig
...
const updateTutorProfile = async (userId, payload) => {
    ...
    try {
        const updatedProfile = await prisma.$transaction(async (tx) => {
            ...
            // Separate payloads
            const { categories, name, image, phone, ...tutorProfileData } = payload;

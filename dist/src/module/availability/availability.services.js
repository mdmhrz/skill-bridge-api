...
const createAvailability = async (availability) => {
    const { tutorProfileId, dayOfWeek, startTime, endTime, } = availability;
    // Validation: required fields
    if (!tutorProfileId || !dayOfWeek || !startTime || !endTime) {
        throw new AppError(httpStatus.BAD_REQUEST, "Tutor profile, day, start time and end time are required");
    }
    // Time validation
    ...
    if (existingAvailability) {
        throw new AppError(httpStatus.CONFLICT, "Availability already exists for this time slot");
    }
    // Create availability
...
const updateAvailability = async (availabilityId, userId, payload) => {
    // validation
    if (!availabilityId) {
    ...
    if (availability.tutorProfile.userId !== userId) {
        throw new AppError(httpStatus.FORBIDDEN, "You are not allowed to update this availability");
    }
    const { dayOfWeek, startTime, endTime, isActive, } = payload;
    ...
    if (overlap) {
        throw new AppError(httpStatus.CONFLICT, "Availability overlaps with an existing time slot");
    }
